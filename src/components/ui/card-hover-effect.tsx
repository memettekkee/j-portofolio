        ...
        <div className={cn("grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 py-10", className)}>
            {items.map((item, idx) => (
                <Link href={item?.link} key={item?.link} className="relative group  block p-2 h-full w-full" onMouseEnter={() => setHoveredIndex(idx)} onMouseLeave={() => setHoveredIndex(null)}>
                    ...
                    <Card>
                        <Image src={item?.url} alt={item?.title} width={1920} height={1080} className="w-full rounded-md" />
...
export const Card = ({ className, children }: { className?: string; children: React.ReactNode }) => {
    return (
        <div className={cn("rounded-2xl h-full w-full p-4 overflow-hidden bg-articleCard border border-transparent dark:border-white/[0.2] group-hover:border-slate-700 relative z-20", className)}>
...
export const CardTitle = ({ className, children }: { className?: string; children: React.ReactNode }) => {
    return <h4 className={cn("mt-4 text-thrdColor font-bold tracking-wide md:text-lg text-center", className)}>{children}</h4>;
};
